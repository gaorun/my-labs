...
> - **orderType**：仅在运单新建页的**调度信息**模块中传入。用户在调度信息区选择运单方式后，填写运费时自动触发服务费计算，此时传入 orderType。
>   - 运单方式选择"满运宝不指派"时，传 orderType = 1
>   - 运单方式选择"满运宝指派"时，传 orderType = 2
>   - 选择"线下调车"或"企普"时不传该参数
>   - 发布货源、编辑货源、运单管理的派车单费用模块中调用该接口时均不传 orderType
