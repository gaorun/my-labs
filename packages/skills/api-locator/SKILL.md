...
> 以下信息仅供前端开发核对结果使用，请勿删除或修改。如 AI 分析结果与实际情况有出入，前端可根据以下路径快速定位相关代码手工修正。

- 调用位置：src/views/xxx/yyy.ts（fetchData 函数）
- 调用位置：src/views/xxx/hooks.ts（useXxx 中传入参数）
- 页面入口：src/views/xxx/CreatePage.vue（某某新建页）
- 页面入口：src/views/xxx/EditPage.vue（某某编辑页）
```
...
- ❌ 函数名/变量名（fetchData、props、formType、isOpen）
...
> fetchData 函数（在 io.js 和 editIo.js 中定义）接受第三个参数 props，通过展开运算符 ...props 混入到 query 参数中。formType 就是通过这个渠道传进去的。
>
> if (this.form.mode === '2') {
> params.formType = 2;
> }
...
=======
> - formType：仅在新建订单页的费用信息模块中传入。用户选择订单类型后，填写费用时自动触发计算，此时传入 formType。
>   - 选择A模式时，传 formType = 1
>   - 选择B模式时，传 formType = 2
>   - 选择C模式或D模式时不传该参数
>   - 列表页、编辑页、详情页的费用模块中调用该接口时均不传 formType
>>>>>>> e1e5782 (docs(AGENTS.md): 新增示例脱敏设计原则)
...
- ✅ **不用表格**：Q&A 模式的回答将直接被复制到钉钉/微信等 IM 工具中，表格无法正常显示。入参来源、出参消费等结构化信息一律用列表格式，不用 markdown 表格
- ✅ **不用代码块**：绝对不要在输出中贴代码。后端和测试不懂前端代码
- ✅ **前端参考附录**：正文中禁止出现的文件路径、函数名等，统一放到输出末尾的前端参考章节。这是输出中唯一允许出现前端技术信息的位置，目的是让前端同事能快速定位代码核验 AI 的分析结果
...
> 以下信息仅供前端开发核对结果使用。如 AI 分析结果与实际情况有出入，前端可根据以下路径快速定位相关代码手工修正。
...
- **合规性检查用穷举法**：逐字扫描草稿中是否出现文件名（含 `.js`/`.vue`/`.ts` 后缀）、函数名（驼峰命名）、代码块（三个反引号）、代码运算符（`===`/`!==`/`&&`/`||`）。前端参考章节除外。出现任何一个，标记为致命问题
