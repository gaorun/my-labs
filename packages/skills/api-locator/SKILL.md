...
- **出参消费**：
  ...
  | data.total | 分页组件显示总条数 | — | 不下发时隐藏总条数 |
...
- ❌ 前端变量名（`tableData`、`orderList`、`loading`）
- ❌ 前端框架术语（`mounted`、`useEffect`、`watch`、`dispatch`、`ref`）
- ❌ 路由配置字段（`meta.title`、`component`、`path`）
- ❌ 代码路径（`src/views/order/index.vue`）
- ❌ 技术实现细节（"用 axios 发送请求"、"在拦截器中处理"）
...
4. **生成报告**：按下面结构输出 Markdown 文件
5. **保存报告**：写入 `{接口名}_api_report.md`
